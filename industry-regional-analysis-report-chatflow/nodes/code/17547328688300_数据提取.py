...
def main(raw_input: Any) -> Dict[str, Any]:
    """
    智能去转义处理节点 (V3.3 - 顺序无关/整体解析版)
    解决 V3.1 因 LLM 输出字段顺序不一致导致提取失败的问题。
    """
    if not isinstance(raw_input, str):
        input_str = str(raw_input)
    else:
    ...
    if not input_str.strip():
        # 如果输入为空，返回空模板
        return {
            "cleaned_object": CONFIG["expected_json_template"],
            "cleaned_string": json.dumps(CONFIG["expected_json_template"], ensure_ascii=False)
        }
    ...
    # 1.1 尝试直接使用 json_repair 修复并加载
    # json_repair 非常强大，能自动处理缺少逗号、引号未闭合等问题
    try:
        parsed_data = json_repair.loads(input_str)
    except Exception:
        # 1.2 如果失败（例如输入包含大量 Markdown 杂音），尝试提取第一个 { 和最后一个 } 之间的内容再解析
        try:
            start_index = input_str.find('{')
            end_index = input_str.rfind('}')
            if start_index != -1 and end_index != -1:
                json_candidate = input_str[start_index : end_index + 1]
                parsed_data = json_repair.loads(json_candidate)
        except Exception:
            # 实在解析不了，打印日志或保持为空
            pass

    # 确保解析出来的是字典，如果解析成了列表或字符串，重置为空
    if not isinstance(parsed_data, dict):
        parsed_data = {}

    # =======================================================
    # 步骤 2: 将解析的数据映射回模板，并针对性清洗字符串
    # =======================================================
    cleaned_object = template.copy()
    ...
    for key, default_val in template.items():
        ...
        if key in parsed_data:
            ...
            # 特殊处理：如果这个值是字符串（比如 text 字段），
            # 这里正是这种 Markdown 字符串可能包含双重转义代码块的地方。
            # 我们调用 V3.1 的核心清洗函数。
            if isinstance(val, str):
                cleaned_object[key] = _clean_markdown_string_aggressively(val)
            else:
                # 如果是列表（references）或数字（confirm），直接使用
                cleaned_object[key] = val
        else:
            # 如果 LLM 没吐出这个 Key，保留模板默认值
            cleaned_object[key] = default_val
